...
import { PointBalanceSummary, presentCoinTerminology } from "./point-purchase-page";
...
export function PointHistoryPage() {
  ...
  if (session.status === "loading" || state.status === "idle" && session.status === "authenticated" && client) {
    return <CatalogLoading label="コイン履歴を読み込み中" />;
  }
  ...
  if (session.status === "configuration-unavailable" || !client) {
    return <CatalogMessage description="この環境ではコイン履歴への接続が設定されていません。" eyebrow="CONFIGURATION" title="コイン履歴を表示できません" />;
  }
  if (session.status === "error") {
    return <CatalogMessage description="Sessionを確認できませんでした。時間をおいて再度お試しください。" eyebrow="ERROR" title="コイン履歴を表示できません" tone="error" />;
  }
  if (state.status === "loading" || state.status === "idle" || state.sessionUserId !== sessionUserId) {
    return <CatalogLoading label="コイン履歴を読み込み中" />;
  }
  if (state.status === "error") {
    return <CatalogMessage action={() => { setState({ status: "loading" }); setRequestKey((value) => value + 1); }} description={state.problem.message} eyebrow="ERROR" title="コイン履歴を取得できませんでした" tone="error" />;
  }
  ...
  return (
    <div className="point-history-page">
      ...
      <section aria-labelledby="point-history-title" className="point-history">
        <header><p>COIN HISTORY</p><h2 id="point-history-title">コイン履歴</h2></header>
        {state.items.length === 0 ? (
          <CatalogMessage description="現在表示できるコイン履歴はありません。" eyebrow="EMPTY" title="コイン履歴はありません" />
        ) : (
          <ol className="point-history__list">
            {state.items.map((entry) => (
              <li key={entry.id}>
                <div><strong>{presentCoinTerminology(entry.reason.label)}</strong><time dateTime={entry.occurred_at}>{formatOccurredAt(entry.occurred_at)}</time></div>
