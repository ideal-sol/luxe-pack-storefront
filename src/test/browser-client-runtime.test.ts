...
import { createBrowserExternalIdentityClient } from "@/lib/platform/external-identity-client";
...
import { PUBLIC_LINE_FRIEND_STATE_FIXTURES } from "@oripa/storefront-testkit";
...
describe("browser Platform transport", () => {
  ...
  it("keeps LINE Friend State reads receiver-safe", async () => {
    const originalFetch = globalThis.fetch;
    const receiverSensitiveFetch = vi.fn(function (this: unknown) {
      if (this !== globalThis) throw new TypeError("Illegal invocation");
      return Promise.resolve(Response.json(PUBLIC_LINE_FRIEND_STATE_FIXTURES.confirmed));
    });
    globalThis.fetch = receiverSensitiveFetch as typeof globalThis.fetch;
    ...
    try {
      ...
      await expect(client.getLineFriendState()).resolves.toMatchObject({
        data: PUBLIC_LINE_FRIEND_STATE_FIXTURES.confirmed,
        metadata: { status: 200 },
      });
      expect(receiverSensitiveFetch).toHaveBeenCalledOnce();
    } finally {
