...
const vendor = path.join(root, "vendor/oripa/STORE-SITE-034");
const predecessorVendor = path.join(root, "vendor/oripa/MIG-063B");
const bundleVersion = "2.0.0-alpha.24";
const predecessorVersion = "2.0.0-alpha.23";
const sourceCommit = "209252d9fcbad42090677f5a7bece52c5a5d3597";
const expected = new Map([
  ["SHA256SUMS", "0cca70f81fea877e6fca898a58fb743c0208d7024c7b2f55743bda3fd40ac27d"],
  ["artifact-manifest.json", "f71edc9e1c9e9215381d01b00ca066ff8bd2678e8cad92d28fce5981145aad94"],
  ["oripa-storefront-client-2.0.0-alpha.24.tgz", "fbe156fbbc9f27a07e4017cc9bea3a9cdcd71aa2943e03fb48236bb48bbda259"],
  ["oripa-storefront-testkit-2.0.0-alpha.24.tgz", "3dc1c3488342846580a2a75372f5d9fff8a510b29d1fad2db468e7276b9efc78"],
...
const publishedPackages = new Map([
  ["@oripa/storefront-client", {
    file: "oripa-storefront-client-2.0.0-alpha.24.tgz",
    version: bundleVersion,
  }],
  ["@oripa/storefront-testkit", {
    file: "oripa-storefront-testkit-2.0.0-alpha.24.tgz",
    version: bundleVersion,
  }],
]);
...
for (const file of [
  "oripa-storefront-client-2.0.0-alpha.24.tgz",
  "oripa-storefront-testkit-2.0.0-alpha.24.tgz",
  "public.openapi.json",
]) {
...
if (manifest.task_id !== "STORE-SITE-034" || manifest.source_commit !== sourceCommit) {
  throw new Error("Artifact provenance mismatch");
}
if (manifest.bundle?.version !== bundleVersion || manifest.bundle?.predecessor !== predecessorVersion ||
...
if (manifest.public_openapi?.file !== "public.openapi.json" ||
    manifest.public_openapi?.version !== predecessorVersion ||
...
if (schemaEntry?.disposition !== "referenced" || schemaEntry.file !== undefined ||
    ...
    schemaEntry.source_bundle_version !== predecessorVersion ||
...
for (const value of [bundleVersion, predecessorVersion, sourceCommit, "STORE-SITE-034", ...expected.values(), referencedSiteSchema.sha256]) {
...
if (clientPackage.oripaCompatibility?.minimumPublicApiContract !== predecessorVersion) {
...
if (testkitPackage.dependencies?.["@oripa/storefront-client"] !== bundleVersion ||
    testkitPackage.dependencies?.["@oripa/site-schema"] !== predecessorVersion ||
    testkitPackage.oripaCompatibility?.storefrontClientVersion !== bundleVersion ||
    testkitPackage.oripaCompatibility?.siteSchemaVersion !== predecessorVersion) {
...
if (schemaPackage.name !== referencedSiteSchema.name || schemaPackage.version !== predecessorVersion) {
...
const predecessorOpenApi = readFileSync(path.join(predecessorVendor, "public.openapi.json"));
if (!currentOpenApi.equals(predecessorOpenApi)) {
  throw new Error("Referenced Public OpenAPI differs from MIG-063B alpha.23");
}
const openApi = JSON.parse(currentOpenApi.toString("utf8"));
if (openApi.info?.version !== predecessorVersion || !openApi.paths?.["/contact-inquiries"]?.post) {
  throw new Error("Referenced Public OpenAPI identity or Contact operation mismatch");
}
